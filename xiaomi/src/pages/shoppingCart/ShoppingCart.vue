<template>
	<div class="broadcast">
		...
		<div class="submit">
			...
			<div class="collection"><butotn v-show="deleteFlag">移入收藏</butotn></div>
			<div class="btn">
				<button :class="{complete:deleteFlag}">{{deleteFlag?"删除":"结算"}} <lebel v-show="!deleteFlag">(<span>{{sumNum}}</span>)</lebel></button>
			</div>
		</div>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			deleteIcon() {
				this.deleteFlag ? this.goodsList.map(item => (item.flag = true)) === undefined : this.goodsList.map(item => (item.flag = false)) !== undefined;
